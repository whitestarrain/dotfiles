...
local configs = require "wsain.treesitter.configs"
local shell = require "wsain.treesitter.shell_command_selectors"
local compat = require "wsain.treesitter.compat"
...
function M.update(options)
  ...
  return function(...)
    ...
    if ... and ... ~= "all" then
      ...
      local parsers_to_update = outdated_parsers() or parsers.installed_parsers()
...
function M.uninstall(...)
  if vim.tbl_contains({ "all" }, ...) then
    reset_progress_counter()
    local installed = parsers.installed_parsers()
