...
local queries = require "wsain.treesitter.query"
local ts = require "wsain.treesitter.compat"
local parsers = require "wsain.treesitter.parsers"
local utils = require "wsain.treesitter.utils"
local caching = require "wsain.treesitter.caching"
...
local builtin_modules = {
  highlight = {
    module_path = "wsain.treesitter.highlight",
  ...
  incremental_selection = {
    module_path = "wsain.treesitter.incremental_selection",
    enable = false,
    keymaps = {
  ...
  indent = {
    module_path = "wsain.treesitter.indent",
    enable = false,
...
local function enable_mod_conf_autocmd(mod)
  ...
  api.nvim_create_autocmd("FileType", {
    ...
    callback = function(args)
      require("wsain.treesitter.configs").reattach_module(mod, args.buf)
    end,
...
function M.setup(user_data)
  ...
  if config.auto_install then
    require("wsain.treesitter.install").setup_auto_install()
  end

  local ensure_installed = user_data.ensure_installed or {}
  if #ensure_installed > 0 then
    if user_data.sync_install then
      require("wsain.treesitter.install").ensure_installed_sync(ensure_installed)
    else
      require("wsain.treesitter.install").ensure_installed(ensure_installed)
    end
  end
